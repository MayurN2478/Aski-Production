...
export default function Approach() {
  return (
    <section
      id="approach"
      className="py-24 md:py-36 bg-charcoal relative overflow-hidden border-b border-white/5"
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 space-y-4">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif font-light text-white leading-tight">
            ...
            <span className="font-serif italic text-gold font-normal">Workflow</span>
          </h2>
          <p className="text-gray-400 text-sm md:text-base font-light max-w-xl mx-auto">
            How we translate your personal milestones into timeless visual masterpieces, structured in three distinct phases.
          </p>
        </div>

        {/* Steps Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-0 lg:border border-white/5 bg-black/20 rounded-sm overflow-hidden">
          {steps.map((step, index) => (
            <motion.div
              ...
              className={`p-8 md:p-12 relative flex flex-col justify-between ${
                index !== 2 ? "lg:border-r lg:border-white/5" : ""
              }`}
            >
              {/* Giant number watermark */}
              <div className="absolute top-8 right-8 text-7xl md:text-8xl font-serif italic font-extralight text-white/5 select-none pointer-events-none">
                {step.num}
              </div>
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs md:text-sm font-light leading-relaxed mb-8 text-left">
                  {step.description}
                </p>
              </div>
              ...
              <div className="border-t border-white/5 pt-6 mt-auto">
                <ul className="space-y-3 text-left">
                  {step.bullets.map((bullet) => (
                    <li key={bullet} className="flex items-center gap-3 text-[11px] md:text-xs text-gray-300 font-light">
                      <svg
